...
*/
    ...
    @IBOutlet weak var enterFibPlace: UITextField!
    
    @IBOutlet weak var showFibPlace: UILabel!

    
    @IBAction func calcFibButton(sender: AnyObject) {
        var fibResult = FibonacciAdder()
        var indexOfFibonacciNumber = enterFibPlace.text.toInt()!
        var result = fibResult.fibonacciNumberAtIndex(indexOfFibonacciNumber)
        showFibPlace.text = "\(result)"
        }
}